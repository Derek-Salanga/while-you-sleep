...
- **n8n Workflow 2, the weekly recap email (2026-09-19):** published and
  live (real schedule: Sunday 20:00 UTC). Both partner inboxes confirmed
  receiving a real, correctly personalized recap letter generated by
  Gemini from `get_weekly_recap_batch()`'s output. Error handling confirmed
  via a deliberate break on one Resend node — the Telegram alert fired with
  the real error, and the *other* partner's email still sent successfully,
